...
impl Default for Esp32CamApp {
    fn default() -> Self {
        Self {
            ...
            last_raw_jpeg: None,
            status_msg: "Ready. Enter ESP32 IP address or use local simulator.".to_string(),
            notification: None,
            fps: 0.0,
            frame_count: 0,
...
impl Esp32CamApp {
    ...
    fn capture_snapshot(&mut self) {
        ...
        // Determine pictures path: prefer /home/computer/pictures, fallback to user pictures dir
            ...
            Ok(n) => {
                ...
                while let Some(soi) = find_subslice(&buffer, &[0xFF, 0xD8]) {
                    if let Some(relative_eoi) = find_subslice(&buffer[soi..], &[0xFF, 0xD9]) {
                        ...
                        // Wait for more bytes for complete frame
                        break;
                    }

                    // Keep buffer size manageable if stream drops corrupted frames
...
impl eframe::App for Esp32CamApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::TopBottomPanel::top("control_panel").show(ctx, |ui| {
            ui.horizontal(|ui| {
                ui.label("IP / Host:");
                ui.add(egui::TextEdit::singleline(&mut self.ip_address).desired_width(180.0));

                ui.label("Path:");
                ui.add(egui::TextEdit::singleline(&mut self.stream_path).desired_width(90.0));
            ...
            if let Some(ref texture) = self.current_texture {
                ...
                ui.centered_and_justified(|ui| {
                    ui.vertical_centered(|ui| {
                        ...
                        ui.add_space(10.0);
                        ui.label("Enter ESP32-CAM IP and click Connect, or launch local simulator.");
                        ui.add_space(15.0);
                        ui.label(egui::RichText::new("Shortcut: Press Ctrl + X to take a snapshot anytime!").italics());
                    });
                });
            }
        });
...
fn main() -> eframe::Result<()> {
    let options = eframe::NativeOptions {
        viewport: egui::ViewportBuilder::default()
            .with_title("ESP32-CAM Streamer & Capture (Ctrl+X)")
